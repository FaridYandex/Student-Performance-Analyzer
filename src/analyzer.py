...
def get_class_statistics(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    return {
        ...
        "risk_distribution": {
            "high": len(df.groupby('student_id').filter(
                lambda x: (100 - x['grade'].mean()) * 0.6 + (100 - x['attendance'].mean()) * 0.4 > 60
            )['student_id'].unique()),
            "medium": len(df.groupby('student_id').filter(
                lambda x: 30 < (100 - x['grade'].mean()) * 0.6 + (100 - x['attendance'].mean()) * 0.4 <= 60
            )['student_id'].unique()),
            "low": len(df.groupby('student_id').filter(
                lambda x: (100 - x['grade'].mean()) * 0.6 + (100 - x['attendance'].mean()) * 0.4 <= 30
            )['student_id'].unique())
        }
    }
